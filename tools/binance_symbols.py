...
if __name__ == '__main__':
	json_obj = http_request_json('https://api.binance.com/api/v1/exchangeInfo')
	support_symbols = json_obj['symbols']

	with open(DEFAULT_OUTPUT_FILE, 'wr') as out_file:
		for index in range(0, len(check_symbol_list)):
			symbol = check_symbol_list[index]
			matches = filter(lambda x: x['baseAsset'] == symbol and x['quoteAsset'] in DEFAULT_WATCH_SYMBOLS, support_symbols)
			ret = '/'.join([match['quoteAsset'] for match in matches])
			if not ret:
				ret = 'NA'
