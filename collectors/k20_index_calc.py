...
class collector_k20_index_calc(collector):
    ...
    def translate_ranks(this, objs):
        ...
        this.logger.debug('k20 calc info - k20_daily_rank query return result length: %s', len(objs['series']))

        for obj in objs['series']:
            if len(obj['values']) < 1:
                this.logger.error('K20 cac Error - k20_daily_rank query return result length 0: %s', objs['values'])
                continue
            ...
            if rank.market_cap_usd <= 0:
                this.logger.error('k20 calc Error - k20_daily_rank query result market cap is incorrect! %s, %s', rank.symbol, rank.market_cap_usd)
                continue
            k20_ranks.append(rank)

            this.logger.debug('k20 calc - rank object generated from DB query: %s, %s, %s', rank.time, rank.symbol, rank.market_cap_usd)
        this.logger.debug('k20 calc - length of rank object generated from DB query: %s', len(k20_ranks))
    ...
    def translate_ticks(this, objs):
        ...
        this.logger.debug('k20 calc - market_ticks query return result length: %s', len(objs['series']))

        for obj in objs['series']:

            if len(obj['values']) < 1:
                this.logger.error('k20 calc Error - market_ticks query return result length 0: %s', objs["values"])
                continue
            ...
            if tick.market == '' or tick.symbol == '':
                this.logger.error('k20 calc Error - market_ticks query result Market/Symbol is incorrect! %s, %s', tick.market, tick.symbol)
                continue

            ticks.append(tick)

            this.logger.debug('k20 calc - Tick object generated from DB query: %s, %s, %s, %s, %s, %s, %s', tick.time, tick.market, tick.symbol, tick.high, tick.low, tick.open, tick.close)
        this.logger.debug('k20 calc - length of Tick object generated from DB query: %s', len(ticks))
    ...
    def collect_rest(this):
        ...
        if len(rank_result) == 0:
            this.logger.error('k20 calc Error - k20_daily_rank table has no data, calculation failed, program exit')
            return
        #print('-------', len(rank_result['series']))
        #print('-------', rank_result['series'][0]['values'])
        ranks = this.translate_ranks(rank_result)
        if len(ranks) == 0:
            this.logger.error('k20 calc Error - translate k20 rank object failed for: %s, program exit', rank_result)
            return
        ...
        this.logger.debug('k20 calc - length of rank object generated: %s', cal_length)
        total_high_weight = total_low_weight = total_open_weight = total_close_weight = 0

        for rank in ranks:

            tick_symbol = this.getSymbol(rank.symbol)
            this.logger.debug('k20 calc - get mapping Tick symbol: %s', tick_symbol)
            if tick_symbol == None:
                this.logger.debug('k20 calc - Warning: New symbol %s  found on top 20, no price collected, bypass it!', rank.symbol)
                continue
            ...
            if len(tick_result) == 0 or tick_result['series'][0]['values'][0][1] == None:
                this.logger.warn('k20 calc - Warning: market_ticks table has no price for symbol: %s  bypass it!', tick_symbol)
                cal_length = cal_length - 1
                continue
            ticks = this.translate_ticks(tick_result)
            if len(ticks) == 0:
                this.logger.warn('k20 calc - Warning: translate ticks failed for: %s  bypass it', tick_result)
                cal_length = cal_length - 1
                continue
        ...
        this.logger.debug('k20 calc - index.timezone_offset: %s', index.timezone_offset)
        this.logger.debug('k20 calc - index.high: %s', index.high)
        this.logger.debug('k20 calc - index.low: %s', index.low)
        this.logger.debug('k20 calc - index.open: %s', index.open)
        this.logger.debug('k20 calc - index.close: %s', index.close)
        this.logger.debug('k20 calc - index.time: %s', index.time)
        this.logger.debug('k20 calc - index.period: %s', index.period)
    ...
    def fillRatio(this, ranks):
        ...
        for rank in ranks:
            ...
            this.logger.debug('k20 calc - rank object symbol: %s  Cap Ratio: %s', rank.symbol, rank.cap_ratio)
        return ranks
    ...
    def calculateSymbolAvgPrice(this, ticks, price_field):
        ...
        this.logger.debug('k20 calc - Calculated Avg Price %s Is: %s for symbol: %s', price_field, avg_price, ticks[0].symbol)
        return avg_price
